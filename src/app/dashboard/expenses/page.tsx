...
export default function ExpensesPage() {
  ...
  const [testingVendor, setTestingVendor] = useState(false);
  ...
  async function loadData(
    ...
    const salaryCategory =
      builtInCategories.get("salary");

    const feeCategory =
      builtInCategories.get("fee");
      ...
      const backfillResults =
        await Promise.all(
          uncategorized.map((expense) => {
            const isSalary =
              expense.invoice_number?.startsWith(
                "PAYROLL-",
              ) ||
              expense.description
                ?.toLowerCase()
   ...
   * Creates:
   * Sri Lakshmi Books
   * Vendor Code: TEST-SLB001
   *
   * This uses the authenticated application
   * session, not SQL Editor.
   */
  async function testCreateVendor() {
    if (!schoolId) {
      setError(
        "School ID not loaded.",
      );
      return;
    }
    ...
    try {
      ...
      if (error) {
        console.error(
          "CREATE VENDOR RPC ERROR:",
          error,
        );

        setError(
          `Create vendor failed: ${error.message} | Code: ${
            error.code || "unknown"
          }`,
        );
      ...
      if (!vendorId) {
        setError(
          `Vendor RPC completed but no vendor_id was returned. Response: ${JSON.stringify(
            data,
          )}`,
        );
      ...
      setSuccess(
        `Test vendor created successfully. Vendor ID: ${vendorId}`,
      );
    } catch (e: any) {
      console.error(
        "CREATE VENDOR EXCEPTION:",
        e,
      );

      setError(
        e?.message ||
          "Unable to create test vendor.",
      );
    } finally {
  ...
  async function createExpense() {
    ...
    if (
      ...
      setError(
        "Date, category, expense account and paid-from account are required.",
      );
      return;
    }
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                ...
                register and accounting details.
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                disabled={testingVendor}
                className="inline-flex items-center gap-2 rounded-lg border border-purple-300 bg-purple-50 px-4 py-2.5 text-sm font-semibold text-purple-700 hover:bg-purple-100 disabled:opacity-50"
              >
                {testingVendor ? (
                  <>
                    <Loader2
        ...
        >
          <div className="flex gap-2">
            ...
            <button
              ...
              className="rounded-lg bg-blue-600 px-5 text-sm font-semibold text-white"
            >
              {savingCategory
                ? "Saving..."
                : "Add"}
        ...
        >
          <div className="grid gap-4 rounded-xl bg-slate-50 border p-4 sm:grid-cols-2">
            ...
            <Info
              label="Category"
              value={
                viewExpense.expense_category_id
                  ? categoryMap.get(
                      viewExpense.expense_category_id,
                    ) || "-"
                  : "Uncategorized"
