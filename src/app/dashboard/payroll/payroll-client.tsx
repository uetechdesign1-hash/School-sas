...
export default function PayrollPage() {
  ...
  async function savePayroll(): Promise<PayrollRun | null> {
    ...
    try {
      ...
      employeeForStaffRef.current = employeeForStaff;
  ...
  async function preparePayment() {
    ...
    if (totals.net <= 0) {
      setError("There is no positive net payroll amount.");
      return;
    }
    ...
    try {
      /*
       * If the payroll is already prepared, the Salary Expense -> Salary
       * Payable journal already exists. In that case we MUST NOT call the
       * preparation RPC again. Instead, recover the existing journal and
       * open Accounting Payment so an admin can continue paying employees
       * individually by Cash or Bank.
       */
      if (locked) {
        const { data: preparedTransaction, error: preparedTransactionError } =
          await supabase
            .from("transactions")
        ...
        if (preparedTransactionError) {
          throw new Error(
            `Unable to recover prepared salary journal: ${preparedTransactionError.message}`,
          );
        }
        ...
        const { data: savedItems, error: savedItemsError } =
          await supabase
            .from("payroll_items")
            .select(
              "employee_id, staff_id, net_salary, paid, paid_amount, status",
            )
            .eq("school_id", schoolId)
            .eq("payroll_run_id", savedRun.id);
        ...
        const remainingAmount = (savedItems || []).reduce(
          (sum, item) =>
            sum +
            Math.max(
        ...
        const payload = {
          ...
          particulars: `Salary Payroll - ${monthText(month)}`,
        ...
        window.location.assign(
          `/accounting/payment?source=payroll` +
            `&run_id=${encodeURIComponent(savedRun.id)}` +
       ...
       *   Cr Salary Payable
       *
       * Bank/Cash is NOT touched here.
       */
      let runToPrepare = savedRun;

      if (editing || !runToPrepare?.id) {
        const saved = await savePayroll();
      ...
      const staffItems = rows
        .map((r) => ({
          ...
          name: staffName(r.staff),
          designation: r.staff.designation || "",
          net_pay: Number(r.net.toFixed(2)),
        }))
        .filter(
          (item): item is {
            employee_id: string;
      ...
      const payload = {
        source: "payroll",
        school_id: schoolId,
        payroll_run_id: runToPrepare.id,
        payroll_month: month,
        amount: payrollAmount,
        particulars: `Salary Payroll - ${monthText(month)}`,
        reference: `PAYROLL-${month}`,
      ...
      setSavedRun(preparedRun);
      setEditing(false);

      const paymentUrl =
        `/accounting/payment?source=payroll` +
        `&run_id=${encodeURIComponent(runToPrepare.id)}` +
        `&month=${encodeURIComponent(month)}` +
        `&amount=${encodeURIComponent(payrollAmount.toFixed(2))}` +
        `&particulars=${encodeURIComponent(payload.particulars)}` +
        `&reference=${encodeURIComponent(payload.reference)}` +
        `&payable_account_id=${encodeURIComponent(
      ...
      console.error("PREPARE PAYROLL ERROR", e);
