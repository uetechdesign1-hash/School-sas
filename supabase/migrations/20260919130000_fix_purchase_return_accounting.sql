-- ============================================================
-- Purchase returns: fix "function lower(account_type) does not exist" (42883)
-- ------------------------------------------------------------
-- ROOT CAUSE
--   On this database public.accounts.account_type is the legacy
--   account_type ENUM, not text. That is documented (and supported) by
--   20260911230000_align_canonical_accounting_legacy_schema.sql, which adds
--   the canonical uppercase labels to that enum and writes its check
--   constraint as lower(account_type::text). lower() only accepts
--   text/varchar, so every uncast comparison inside the purchase-return
--   functions fails with SQLSTATE 42883
--     "function lower(account_type) does not exist"
--   before any return row, stock movement or journal line is written:
--
--     20260917120000_post_purchase_returns.sql:60,65
--     20260918120000_purchase_returns_history_particulars.sql:84,89
--
--   The fee-payment RPCs hit the exact same bug and were fixed by casting
--   (lower(account_type::text), see 20260911210000). The purchase-return
--   functions were written later but without the cast.
--
-- FIX
--   Re-post the current purchase-return RPC set (identical to
--   20260918120000_purchase_returns_history_particulars.sql) with the two
--   account_type comparisons cast to text. Signatures, accounting model,
--   stock handling, grants and particulars are unchanged; only
--   lower(...account_type...) -> lower(...account_type::text...) differs.
--   The cast is safe on both schema variants: canonical databases store
--   account_type as text (text::text is a no-op), legacy databases store the
--   enum (enum::text is required for lower()).
--
--   No table is dropped, no accounting history is deleted, no data is reset
--   and no unrelated module is touched. Safe to run repeatedly.
-- ============================================================
...
begin
  ...
  -- account_type may be the legacy enum: cast to text before lower().
...
--    (internal helper used by update / delete; unchanged)
-- ------------------------------------------------------------
...
--    (reverse old effect, re-post under the same return id; unchanged)
-- ------------------------------------------------------------
...
--    (reverse effect, drop the return header/items, keep the bill and
...
-- Refresh the PostgREST schema cache for the replaced functions.
